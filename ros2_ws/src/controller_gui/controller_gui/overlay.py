'''
This module contains the logic for all GUI drawing. 
The drawn on image is returned
...
def overlay(img, speed, battery, ballast_left, ballast_right, depth, deg, time_start):
    '''
    Method that actually does the work described above
    '''
    ...
    #pass through.
    ballast_cap_l = np.array([[10, 200], [15, 190], [20, 185], [25, 185],
                              [30, 190],  [35, 200]], np.int32)
    ...
    hull_cap = np.array([[55, 175], [60, 170], [65, 165], [70, 160], [75, 155],
                         [110, 155], [115, 160], [120, 165], [125, 170], [130, 175]], np.int32)
    ...
    ballast_cap_r = np.array([[150, 200], [155, 190], [160, 185], [165, 185],
                              [170, 190], [175, 200]], np.int32)
    ballast_cap_r = ballast_cap_r.reshape((-1, 1, 2))
    ballast_stern_r = np.array([[150, 300], [155, 310], [160, 310], [170, 310],
                                [175, 300]], np.int32)
    ballast_stern_r = ballast_stern_r.reshape((-1, 1, 2))

    ##DRAW BATTERY:
    #Draw rectangle backdrop
    cv.rectangle(img, (battery_bottom_x, battery_bottom_y),
                 (battery_top_x, battery_top_y), (0, 255, 0), -1)
    #Draw rectangle for battery
    cv.rectangle(img, (battery_bottom_x, battery_bottom_y),
                 (battery_top_x, battery_top_y), (255, 255, 255), 3)
    #Draw terminal nib
    cv.rectangle(img, (battery_top_x, 25), (battery_top_x + 10, 35), (255, 255, 255), -1)

    #Next, need to correlate the 2.8V range from full to empty to 0->100%
    #(Battery is fully charged at 17V, dead at 12V)
    #100/2.8 = 35.7. Rectangle is 255 pixels long, so 255/2.8 = 91.07
    #for color gradient.
    #Draw red/green bar
    cv.rectangle(img, (battery_bottom_x, battery_bottom_y),
                (battery_top_x, battery_top_y),  (0 + int((battery * 20) - 240),
                (255 - int((battery * 20) - 240)), 0), -1)

    #Grey bar
    cv.rectangle(img, (battery_bottom_x + int((((battery * 20) - 240))), 10),
                (battery_top_x, 50), (150, 150, 150), -1)

    cv.putText(img, str(int((battery * 20) - 240)).zfill(2), (930, 40),
               cv.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1, cv.LINE_AA)

    ##DRAW COMPASS:
    #Compass base
    cv.rectangle(img, (compass_bottom_x, compass_bottom_y),
                (compass_top_x, compass_top_y), (150, 150, 150), -1)
    cv.rectangle(img, (compass_bottom_x, compass_bottom_y),
                (compass_top_x, compass_top_y), (255, 255, 255), 2)
    #Draw left box
    cv.rectangle(img, (compass_bottom_x, compass_bottom_y),
                (compass_bottom_x + 50, compass_top_y), (255, 255, 255), 2)

    #Draw right box
    cv.rectangle(img, (compass_top_x - 50, compass_bottom_y),
                (compass_top_x, compass_top_y), (255, 255, 255), 2)
    ...
    #The length of the compass box - 100 (the size of the two sidebars)
    #is 300/90 (The desired amount of degrees shown)
    #After a lot of subjective testing, the above does work, but it doesn't
    #look particularly great. This number makes
    #The headings appear much more nicely than before
    scale_deg = 2.18

    directions = ["N", "NW", "W", "SW", "S", "SE", "E", "NE"]
    #Want to display each heading in the box... Need to make it so that
    #everything gets centered on the point that the arrow is on
    ...
    for cardinal in range(8): #each direction label
        ...
        if(degree >= -60 and degree <= 60):
            cv.putText(img, str(directions[cardinal]), (int(degree*scale_deg + 495), 25),
    ...
    if(0 < deg < 44) or (deg == 360):
        cv.putText(img, "N", (663, 25), cv.FONT_HERSHEY_SIMPLEX, .5,
                  (255, 255, 255), 1, cv.LINE_AA)
    if 45 < deg < 89:
        cv.putText(img, "NE", (663, 25), cv.FONT_HERSHEY_SIMPLEX, .5,
                  (255, 255, 255), 1, cv.LINE_AA)
    if 90 < deg < 134:
        cv.putText(img, "E", (663, 25), cv.FONT_HERSHEY_SIMPLEX, .5,
                  (255, 255, 255), 1, cv.LINE_AA)
    if 135 < deg < 179:
        cv.putText(img, "SE", (663, 25), cv.FONT_HERSHEY_SIMPLEX, .5,
                  (255, 255, 255), 1, cv.LINE_AA)
    if 180 < deg < 224:
        cv.putText(img, "S", (663, 25), cv.FONT_HERSHEY_SIMPLEX, .5,
                  (255, 255, 255), 1, cv.LINE_AA)
    if 225 < deg < 269:
        cv.putText(img, "SW", (663, 25), cv.FONT_HERSHEY_SIMPLEX, .5,
                  (255, 255, 255), 1, cv.LINE_AA)
    if 270 < deg < 314:
        cv.putText(img, "W", (663, 25), cv.FONT_HERSHEY_SIMPLEX, .5,
                  (255, 255, 255), 1, cv.LINE_AA)
    if 315 < deg < 359:
        cv.putText(img, "NW", (663, 25), cv.FONT_HERSHEY_SIMPLEX, .5,
                  (255, 255, 255), 1, cv.LINE_AA)
    ...
    cv.putText(img, str(deg), (compass_bottom_x + 10, compass_bottom_y + 15),
              cv.FONT_HERSHEY_SIMPLEX, .5, (255, 255, 255), 1, cv.LINE_AA)
    cv.rectangle(img, (compass_top_x - 50, compass_bottom_y),
                (compass_top_x, compass_top_y), (255, 255, 255), 2)
    ...
    ##DRAW DEPTH:
    cv.putText(img, "Z Acceleration: ", (10, 540), cv.FONT_HERSHEY_SIMPLEX, .5,
              (255, 255, 255), 1, cv.LINE_AA)
    cv.putText(img, str(f"{depth:.3f}"), (135, 540), cv.FONT_HERSHEY_SIMPLEX, .5,
              (255, 255, 255), 1, cv.LINE_AA)
